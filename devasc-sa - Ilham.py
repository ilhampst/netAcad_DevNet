...
if choice == 'n' or 'N':
    token = input('Please input your access token: ')
    accessToken = "Bearer {}".format(token)
else:
    accessToken = "Bearer NjI4YmE1ZTktZmU2YS00MzEyLWE0ZGEtZmU1OGFmZjU1NWQzYjUxYzY3ZmQtNWVk_P0A1_a5f91689-712e-402d-aa9f-0e4101389783"
...
# 9. Provide your MapQuest API consumer key.
        ...
        mapsAPIGetParameters = { 
                                "lat": lat,
                                "lng": lng,
                                "key": "bWhMbhRqaI8GBVxIGJ1OUawnWNMNLA7Z"
                               }
...
# 11. Store the location received from the MapQuest API in a variable
        CountryResult = json_data["results"][0]["locations"][0]["adminArea1"] #adminArea1
        StateResult = json_data["results"][0]["locations"][0]["adminArea3"]#adminArea3
        CityResult = json_data["results"][0]["locations"][0]["adminArea4"]#adminArea4
        StreetResult = json_data["results"][0]["locations"][0]["street"]#street
...
#<!!!REPLACEME with if statements to compose the message to display the current ISS location in the Webex Team room!!!>
        elif not CountryResult == "XZ":
            responseMessage = "On {}, the ISS was flying over the following location: \n{} \n{}, {} \n{}\n({}\", {}\")".format(timeString, StreetResult, CityResult, StateResult, CountryResult, lat, lng)
        else:
            responseMessage = "Currently not enough data to display"
